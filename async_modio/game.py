...
class Game:
    ...
    async def add_media(self, **media):
        ...
        try:
            message = await self._client._post_request(f'/games/{self.id}/media', h_type = 1, files = fields)
        finally:
    ...
    async def add_tags(self, **tag_option):
        ...
        message = await self._client._post_request(f'/games/{self.id}/tags', data = {"input_json" : input_json})
    ...
    async def delete_tags(self, **tags):
        ...
        tags = tags.pop("tags", [])
        tags = {f"tags[{tags.index(tag)}]" : tag for tag in tags}
        r = await self._client._delete_request(f'/games/{self.id}/tags', data = tags)
