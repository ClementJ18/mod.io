...
class Client:
    ...
    loop : Optional[asyncio.Task]
        The loop of your current asyncio task, will create its own if none are passed.
    ...
    async def _get_request(self, url, **fields):
        filter = (fields.pop("filter") if fields.get("filter") else Filter()).__dict__
        extra = {**filter, **fields}
        ...
        async with self.session.get(self.BASE_PATH + url, headers=self._define_headers(h_type)) as r:
    ...
    async def email_request(self, email : str):
        ...
        async with self.session.post(self.BASE_PATH + "/oauth/emailrequest", params={
          'api_key': self.api_key,
          'email' : email
        }, headers = headers) as r:
            return Message(**await self._error_check(r))
    ...
    async def email_exchange(self, code : int):
        ...
        async with self.session.post(self.BASE_PATH + "/oauth/emailexchange", params={
          'api_key': self.api_key,
          'security_code' : code
        }, headers = headers) as r:
            r = await self._error_check(r)
